...
skipl = "skip"
wks =  r"C:\Project_Nezammahalleh\MyProject7\vol2.gdb" #arcpy.GetParameterAsText(5) #r"D:\Nezammahalleh\project_Nezammahalleh\MyProject3\vol7.gdb"
...
for idx_f in qqf:
    ...
    try:
        ...
        try:
            ...
            if abs((arcpy.SearchCursor(idx_d,"","","","NEAR_DIST"+ " A").next().getValue("NEAR_DIST")) - (arcpy.SearchCursor(idx_c,"","","","NEAR_DIST"+ " A").next().getValue("NEAR_DIST"))) < 14:
        ...
        arcpy.analysis.Select(idx_f, f'skp_{idx_f}', "OBJECTID" + '='+str(1))
        
        
#merging     
qmrg = arcpy.ListFeatureClasses("j*","")
mgskip = arcpy.ListFeatureClasses("skp*","")
arcpy.management.Merge(qmrg, finalmg, field_mappings="", add_source="NO_SOURCE_INFO")
arcpy.management.Merge(mgskip, skipl, field_mappings="", add_source="NO_SOURCE_INFO")

#clearing
arcpy.management.Delete(arcpy.ListFeatureClasses("kd*",""), '')
arcpy.management.Delete(arcpy.ListFeatureClasses("kf*",""), '')
arcpy.management.Delete(arcpy.ListFeatureClasses("kc*",""), '')
arcpy.management.Delete(arcpy.ListRasters("x*",""), '')
arcpy.management.Delete(arcpy.ListFeatureClasses("x*",""), '')
arcpy.management.Delete(arcpy.ListFeatureClasses("skp*",""), '')
arcpy.management.Delete(arcpy.ListFeatureClasses("jd*",""), '')
arcpy.management.Delete(arcpy.ListFeatureClasses("jc*",""), '')
